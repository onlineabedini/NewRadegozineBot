...
const {registered_for_plan_message} = require("../../../messages/studentMessages");
const {
...
const {send_pay_pic_button} = require("../../../buttons/user_buttons/send_pay_pic_button");
...
module.exports = new (class ProStudentService {
    async getProStudentFullName(ctx, next) {
    ...
    async getProStudentField(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
    ...
    async getProStudentGrade(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
    ...
    async getProStudentPhoneNumber(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
    ...
    async getProStudentWhatsUpNumber(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
    ...
    async getProStudentEmail(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
            if (ctx.message.text) {
    ...
    async getProStudentCity(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
            if (ctx.message.text) {
    ...
    async getProStudentPaymentPicture(ctx, next) {
        ctx.session.state = undefined;
        if (ctx.message.text !== all_buttons_text.cancel) {
            if (ctx.message.photo) {
                ...
                if (student) {
                    ...
                    await student.save();
                    await ctx.telegram.sendPhoto(process.env.MAIN_ADMIN_ID, student.userPaymentPicture, {
