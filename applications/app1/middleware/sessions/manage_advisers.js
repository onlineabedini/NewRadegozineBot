...
const {
  ...
  no_adviser_found_with_this_input,
...
module.exports = {
  ...
  [state_list.remove_adviser]: async (ctx, next) => {
    ...
    if (ctx.message.text !== all_buttons_text.cancel) {
      if (ctx.message.text) {
        ...
        const username = user_input.split("@")[1];
        ...
        if (username) {
          ...
          if (adviser) {
            await AdviserModel.findOneAndDelete({ username });
          ...
          if (adviser) {
            await AdviserModel.findOneAndDelete({ chat_id: user_input });
            ctx.reply(adviser_removed_message, manage_advisers_buttons);
            ctx.session = undefined;
          } else {
            ctx.session.state = state_list.remove_adviser;
    ...
    if (ctx.message.text !== all_buttons_text.cancel) {
      if (ctx.message.text) {
        ...
        if (username) {
          ...
          if (adviser) {
            adviser = await AdviserModel.findOneAndUpdate(
              { username },
              {
                is_pro: true,
              },
            ...
            ctx.reply(no_adviser_found_with_this_input);
          }
        } else {
          let adviser = await AdviserModel.findOne({
            chat_id: user_input,
  ...
  [state_list.accept_adviser]: async (ctx, next) => {
    ...
    if (ctx.message.text === all_buttons_text.yes) {
      ...
      if (adviser) {
        adviser = await AdviserModel.findByIdAndUpdate(
          ctx.session.adviser_id,
          {
            is_accepted: true,
          },
          { new: true }
